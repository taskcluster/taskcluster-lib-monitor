...
class Firehose extends events.EventEmitter {

  constructor({aws, streamName, reportFirehoseErrors, resourceInterval, crashTimeout}) {
    super();
    if (!streamName || !aws) {
    ...
    this._records = [];
    this._puts = Promise.resolve();
    this._done = false;
    this._flushTimer = setTimeout(this.flush.bind(this), this._flushInterval);
    this._reportErrors = reportFirehoseErrors;

    process.on('SIGTERM', this._cleanup.bind(this));
  }

  async _cleanup() {
    console.log('Beginning cleanup! Trying to submit remaining records to kinesis');
    setTimeout(() => {
      console.log('Failed to report logs to Kinesis after timeout!');
      process.exit(1);
    }, this._crashTimeout);
    await this.close();
  ...
  async setup() {
    let stat = await this._firehose.describeDeliveryStream({
      DeliveryStreamName: this._streamName,
    }).promise();
    if (stat.DeliveryStreamDescription.DeliveryStreamStatus !== 'ACTIVE') {
      throw new Error(`Delivery Stream ${this._streamName} is not yet active!`);
    }
  }

  async close() {
    this._done = true;
    while (this._records.length) {
      this.flush();
      await this._puts;
    }
  ...
  log(record) {
    ...
    let recordCount = this._records.push({line, retries: 0, length});
    if (recordCount >= MAX_PUT_RECORDS) {
      // May as well get a headstart if we're getting flooded.
      this.flush();
    }
  }

  async flush() {
    this._puts = this._puts.then(async () => {
      if (this._flushTimer) {
      ...
      await Promise.map(chunks, async records => {
        ...
        res.RequestResponses.forEach((req, i) => {
          if (req.ErrorCode) {
            ...
            debug('Failed to write record ' + record + '. Reason: ' + req.ErrorMessage);
...
class NoopFirehose extends events.EventEmitter {
  ...
  log(record) {
    throw new Error('Must give credentials and stream name to lib-monitor to use structured logging!');
  }
...
module.exports = {NoopFirehose, Firehose};
